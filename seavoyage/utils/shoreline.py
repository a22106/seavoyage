...
import os
from shapely.geometry import MultiPolygon, LineString, Point, MultiPoint
from shapely.prepared import prep
from enum import Enum

class ShorelineType(Enum):
    CRUDE = "c"
    LOW = "l"
    INTERMEDIATE = "i"
    HIGH = 'h'
...
def load_land_polygon(level: ShorelineType | str = ShorelineType.LOW) -> MultiPolygon:
    """
    shapefile, gpkg 파일에서 MultiPolygon 객체로 반환
    :param level: ShorelineType Enum 또는 파일 경로
    :return: MultiPolygon 객체
    """
    if isinstance(level, str) and os.path.exists(level):
        file_path = level
    else:
        level_value = level.value if isinstance(level, ShorelineType) else level
        file_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 
                     "data", "GSHHS_shp", level_value, f"GSHHS_{level_value}_L1.shp")
...
def is_valid_edge(
    line: LineString, 
    land_polygon: MultiPolygon = None, 
    ...
    """
    # Cache land_polygon if not provided
    if land_polygon is None:
        land_polygon = getattr(is_valid_edge, '_cached_land_polygon', None)
        if land_polygon is None:
            land_polygon = load_land_polygon(ShorelineType.LOW)
            is_valid_edge._cached_land_polygon = land_polygon
    else:
        # Update cache with the provided land_polygon
        is_valid_edge._cached_land_polygon = land_polygon
